import { loadHeaderFooter, getLocalStorage, renderStars, getLocation, encodeGeohash } from "./utils.mjs";
...
async function renderEvents() {
  ...
  const geoHashString = encodeGeohash(location.lat, location.long, 6);
	...
	const starredEvents = events.filter(event => starredEventIds.indexOf(event.id) !== -1);

	eventsEle.innerHTML = "";
	starredEvents.map((event) => eventTemplate(event)).forEach(eventCard => { eventsEle.appendChild(eventCard) })
